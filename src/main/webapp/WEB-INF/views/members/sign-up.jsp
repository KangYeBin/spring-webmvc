    ...
    <script>
        ...
        $idInput.onkeyup = e => {
            ...
            if (idValue.trim() === '') {
                ...
                // 정규표현식의 함수 test를 통해 입력값이 패턴에 유효한지 검증
        ...
        document.getElementById('signup-btn').onclick = e => {
            ...
            // 5개의 입력칸이 모두 통과되었을 경우 form을 submit
        ...
        // 이미지 영역을 클릭하면 input type=file을 클릭한 것과 동일한 효과를 내도록 한다
        $profile.onclick = e => {
            $fileInput.click();
        }
        ...
        $fileInput.onchange = e => {
            ...
            // 첨부 파일의 바이트 데이터를 읽어들이는 객체 생성
            const reader = new FileReader();

            // 파일의 바이트 데이터를 읽어서 img 태그의 src 속성에 넣으려면 url 형태로 전달해야 하는데
            // 이걸 처리하는 함수 이용
            reader.readAsDataURL(fileData);

            // 파일 리더 객체가 로딩이 끝났다면 이벤트를 발생시켜서 img 태그에 이미지 세팅
            reader.onloadend = e => {
